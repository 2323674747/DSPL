...
class FewShotModel(nn.Module):
    ...
    def forward(self, x, lab_s, lab, get_feature=False, epo=0):
        ...
        if self.training:
            ...
            # logits, logits_reg, bce_loss, loss_rec, loss_rec2, loss_trans, loss_trans2 ,loss_core= self._forward(support_idx, query_idx, emd_v, lab_s, lab, epo)
            ...
            ###################################################
            logits, _ = self._forward(support_idx, query_idx, emd_v, lab_s, lab_s)
            return logits
