...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='MiniImageNet', choices=['MiniImageNet', 'TieredImagenet', 'CUB', 'Domain_FS', 'CUB_RAW'])    
    parser.add_argument('--backbone_class', type=str, default='Res12', choices=['ConvNet', 'Res12', 'Res12f', 'Res18', 'GFNet12', 'ResMLP12','Res12_bdc','Res18_bdc'])
    ...
    parser.add_argument('--tag', type=str, default="128")
    ...
    save_path2 = '_'.join([str(args.lr), str(args.gamma), args.tag])
    ...
    if args.dataset == 'MiniImageNet':
        ...
        # from model.dataloader.tiered_imagenet import tieredImageNet_H as Dataset
    ...
    # val_sampler = CategoriesSampler(valset.label, 200, 5, 1 + args.query) # test on 16-way 1-shot
    ...
    if 'Conv' in  args.backbone_class:
        ...
        #     {'params': model.dcov.temperature, 'lr': 0.001}], lr=args.lr, momentum=0.9, nesterov=True, weight_decay=0.0005)
    elif 'GFNet' in args.backbone_class:
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, nesterov=True, weight_decay=0.0005)
    else:
        ...
        if epoch > 100 or (epoch-1) % 5 == 0:
            ...
            with torch.no_grad():
                for i, batch in tqdm(enumerate(val_loader, 1)):
                    ...
                    # logits_dist, logits_sim = model.forward_proto(data_shot, data_query, 5)
