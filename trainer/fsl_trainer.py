    ...
    def train(self):
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            for batch in self.train_loader:
                ...
                ##########################################################################
                lambda_core = min(1.0, epoch / (args.max_epoch * 0.3))
                # print(lambda_core)
                # logits, reg_logits, bce_loss, loss_rec, loss_rec_self, loss_tran, loss_tran2 ,loss_core = self.para_model(data, label_s, label, False, epoch)
                ...
                if reg_logits is not None:  # For FEAT-based
                    # print(f"logits shape: {logits.shape}, labels shape: {label.shape}")
                    ...
                    # total_loss = loss + lambda_core *loss_core+ 0.01 * bce_loss + 0.1 * loss_rec + 0.1 * loss_rec_self + self.args.balance * loss_reg + loss_tran + loss_tran2
                else:  # For others
                    loss = F.cross_entropy(logits, label)
            ...
            ###########################################################
            vl, va, vap = self.evaluate(self.val_loader, epoch)

            train_losses_all.append(tl1.item())
            val_accs_all.append(va)
        ...
        # 画 train loss 曲线
        plt.subplot(1, 2, 1)
        plt.plot(epochs, train_losses_all, 'r-', label='Train Loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        ...
        # 画 val acc 曲线
        plt.subplot(1, 2, 2)
        plt.plot(epochs, val_accs_all, 'b-', label='Validation Accuracy')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        ...
        plt.tight_layout()
        plt.savefig(osp.join(args.save_path, 'loss_acc_curve.png'))  # 保存图片
        plt.show()
