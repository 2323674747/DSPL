...
def postprocess_args_test(args):            
    args.num_classes = args.way
    save_path1 = '-'.join([args.dataset, args.model_class, args.backbone_class, '{:02d}w{:02d}s{:02}q'.format(args.way, args.shot, args.query)])
    save_path2 = '_'.join([str('_'.join(args.step_size.split(','))), str(args.gamma),
                           'lr{:.2g}mul{:.2g}'.format(args.lr, args.lr_mul),
...
def get_command_line_parser():
    ...
    parser.add_argument('--model_class', type=str, default='FEAT2', 
                        choices=['FEAT2','ProtoNet2', 'FEAT', 'ProtoNet'])
    '''
    parser.add_argument('--model_class', type=str, default='ProtoNet2',
                        choices=['FEAT2', 'ProtoNet2', 'FEAT', 'ProtoNet'])
    '''
    ...
    parser.add_argument('--dataset', type=str, default='MiniImageNet',
                        choices=['MiniImageNet', 'TieredImageNet', 'CUB', 'Domain_FS', 'CUB_RAW'])
    parser.add_argument('--orig_imsize', type=int, default=84,
                        help='Original image size (e.g., 84 for MiniImageNet, 224 for CUB)')
    ...
    parser.add_argument('--balance', type=float, default=0.01)   # only for FEAT and FEAT+DFR
    parser.add_argument('--temperature', type=float, default=64)
    parser.add_argument('--temperature2', type=float, default=64)  # only for FEAT and FEAT+DFR
    ...
    parser.add_argument('--init_weights', type=str, default='/home/aaa/zll/DFRFS-main/initialization/miniimagenet/Res12-pre.pth')
    ...
    parser.add_argument('--save_dir', type=str, default='/home/aaa/zll/DFRFS-main/checkpoints')
    parser.add_argument('--tag', type=str, default=None, help='notes to record different settings')
    parser.add_argument('--tid', type=int, default=0, help='the id of test domain')
    return parser
